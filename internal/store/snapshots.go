...
func (s *Store) CreateSnapshot(ctx context.Context, params CreateSnapshotParams) (Snapshot, error) {
	_, err := s.db.ExecContext(ctx, `
		insert into snapshots (name, source_vm, state_path, mem_path, disk_path, base_image_id, kernel_id, base_image_metadata)
		values (?, ?, ?, ?, ?, ?, ?, ?)
	`, params.Name, params.SourceVM, params.StatePath, params.MemPath, params.DiskPath, params.BaseImageID, params.KernelID, params.BaseImageMetadata)
	if err != nil {
		return Snapshot{}, err
	}
...
func (s *Store) ListSnapshots(ctx context.Context) ([]Snapshot, error) {
	rows, err := s.db.QueryContext(ctx, `
		select name, coalesce(source_vm, ''), state_path, mem_path, disk_path, base_image_id, kernel_id, base_image_metadata, created_at
		from snapshots
		order by name
	`)
	if err != nil {
